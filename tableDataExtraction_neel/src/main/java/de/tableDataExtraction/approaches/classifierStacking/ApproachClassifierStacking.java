...
public class ApproachClassifierStacking implements Approach {

	private static final String COLUMN_FILE_NAME = "approachClassifierStackingColumnFeatures.csv";
	private static final String ROW_FILE_NAME = "approachClassifierStackingRowFeatures.csv";
	...
	public ApproachClassifierStacking(ApproachSettings settings) {

		this.settings = (SettingsClassifierStacking) settings;
	...
	private void learnColumns(Table table) {
		...
		for (Column column : table.getData().getColumns().values()) {
			...
			featureCollection.getColumnFeatures().put(columnId, columnFeatures);
	...
	private void learnRows(Table table) {
		...
		for (Row row : table.getData().getRows().values()) {
			...
			rowFeatures = featureGenerator
					.computeFeatures(table, rowIndex, row);
