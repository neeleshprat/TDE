...
public class TableData implements Serializable {
	...
	public TableData(String inputContentString) {
	...
	private void handleCells(Element soupRow, Row row, int rowNumber, String tag) {
		...
		for (Element sCell : sCells) {
			...
			// go really over all cells after solution of colspan
			for (int i = 0; i < colspan; i++) {

				Cell cell;
				Column column;

				column = determineColumn(rowNumber, columnNumber);
	...
	private Column determineColumn(int rowNumber, int columnNumber) {
